...
* The docker container is built on top of bioconductor/bioconductor_docker:devel and installs the package with the shiny app.
...
# Publishing and deploying the docker image onto GitHub Container Registry (GHCR)

* Push your repository to GitHub on the `devel` branch.

* The GitHub Action defined in the [workflows folder](./.github/workflows/buildpush.yml) will trigger automatically on pushes to the `devel` branch. It will log in to `ghcr.io` automatically using the run credentials, build the image, and push it to the registry.
